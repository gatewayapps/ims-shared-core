...
class PermissionHandler {
  constructor(imsConfig) {
    this.ImsConfig = imsConfig;
  }

  checkPermission(permission, userPermissions, skipTreeNodeCheck) {
    ...
    if (typeof permission === 'string') {
      ...
      permission = this._autoFillValues(permission);
    }
    ...
    if (this._isPermissionDenied(permission, userPermissions))
      return false;


    //Has it been explicitly granted to the user
    if (this._isPermissionGranted(permission, userPermissions, skipTreeNodeCheck))
      return true;


    //Did the user inherit it based on their role
    if (this._isPermissionInherited(permission, userPermissions))
      return true;

    return false;
  }

  checkPermissions(permissions, userPermissions, skipTreeNodeCheck) {
    return permissions.some((p) => {
      return this.checkPermission(p, userPermissions, skipTreeNodeCheck);
    });
  }

  createPermissionFromString(permissionString) {
    ...
    return this._autoFillValues(retVal);
  }

  createPermissionsArrayFromStringsArray(stringsArray) {
  ...
  _autoFillValues(permission) {
    if (!permission.type)
      permission.type = Constants.RoleTypes.Grant;
    if (!permission.package)
      permission.package = this.ImsConfig.package.id;
    if (!permission.role)
  ...
  _isPermissionDenied(permission, userPermissions) {
    for (var i = 0; i < userPermissions.length; i++) {
      const p = userPermissions[i];

      // Is this a deny permission and does it apply to this package
      if (p.type === Constants.RoleTypes.Deny && p.package === this.ImsConfig.package.id) {
  ...
  _isPermissionGranted(permission, userPermissions, skipTreeNodeCheck) {
    for (var i = 0; i < userPermissions.length; i++) {
      const p = userPermissions[i];

      //Is this a grant permission and does it apply to this package
      if (p.type === Constants.RoleTypes.Grant && p.package === this.ImsConfig.package.id) {
  ...
  _isPermissionInherited(permission, userPermissions, skipTreeNodeCheck) {
    skipTreeNodeCheck = skipTreeNodeCheck || false;
