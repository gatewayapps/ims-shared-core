...
export default class Activity {
      ...
      db.collection("TaskActivities").findOne(
        ...
        (err, result) => {
          if (err) {
            ...
            return Activity.LoadCompletedActivitiesForTask(db, result.context.taskId).then(
              results => {
  ...
  requireInput(name) {
    if (this.activity.inputs === undefined || this.activity.inputs[name] === undefined) {
      this.fail(
        `${this.activity.packageId}:${this.context.step} requires input ${name} but was passed undefined`
      );
      return false;
    } else {
      return true;
