  ...
  app.use("/api/core/activity", (req, res, next) => {
    ...
    if (fs.existsSync(activityPath)) {
      ...
      if (activityRef.CreateActivityAsync) {
        Activity.CreateActivityAsync(req.body.context, req.body.activity, dbo, activityRef).then(
          activity => {
        ...
        res.json({
          success: false,
          reason: `Activity '${req.body.activity.id}' does not inherit from ims-shared-core::Activity`
        });
      }
    } else {
      res.json({
        success: false,
        reason: `Activity definition not found for ${req.body.activity.id}`
      });
    }
  });
}
