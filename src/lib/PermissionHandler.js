...
function PermissionHandler (imsConfig) {
  ...
  this.checkPermission = function (permission, userPermissions, skipTreeNodeCheck) {
    ...
    if (_isPermissionDenied(permission, userPermissions)) { return false }

    // Has it been explicitly granted to the user
    if (_isPermissionGranted(permission, userPermissions, skipTreeNodeCheck)) { return true }

    // Did the user inherit it based on their role
    if (_isPermissionInherited(permission, userPermissions, skipTreeNodeCheck)) { return true }
  ...
  this.createPermissionsArrayFromStringsArray = function (stringsArray) {
    ...
    return stringsArray.map(function (permissionString) {
      if (!typeof permissionString === 'string') { return undefined }
  ...
  function _autoFillValues (permission) {
    if (!permission.type) { permission.type = Constants.RoleTypes.Grant }
    if (!permission.package) { permission.package = self.ImsConfig.package.id }
    if (!permission.role) { permission.role = 'user' }
    if (!permission.action) { permission.action = '*' }
    if (!permission.tree) { permission.tree = '*' }
    if (!permission.node) { permission.node = '*' }
